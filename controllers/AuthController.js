...
module.exports = {
    createUser: async(req, res) => {
        ...
        try {
            await admin.auth().getUserByEmil(user.email);
    ...
    loginUser: async(req, res) => {
        try {
            ...
            const decrytedPassword = CryptoJS.AES.decrypt(user.password, process.env.SECRET);
            const decryted = decrytedPassword.toString(CryptoJS.enc.Utf8);

            decryted !== req.body.password && res.status(401).json("Wrong password")
